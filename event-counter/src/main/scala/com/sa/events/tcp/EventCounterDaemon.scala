...
import cats.effect.{Blocker, Bracket, ConcurrentEffect, ContextShift, IOApp, Timer}
...
object EventCounterDaemon {
  ...
  def tcpStream[F[_]](socketGroup: SocketGroup, serverAddr: InetSocketAddress)
                     (implicit F: ConcurrentEffect[F]
                      , timer: Timer[F]
                      , contextShift: ContextShift[F]): Stream[F, Option[List[EventData]]] = {

    val outcome = for {
      res <- Stream.eval{
        F.delay(socketGroup.client[F](serverAddr
  ...
  def init[F[_]](socketGroup: SocketGroup)
          (implicit F: ConcurrentEffect[F]
           , timer: Timer[F]
    ...
    res <- Stream.awakeEvery[F](10 seconds) >> tcpStream(socketGroup, serverAddr)
  } yield ()
...
//    Blocker[F].use { bl =>
      ...
      SocketGroup[F](blocker).use { init(_).compile.drain }
//    }
  }
